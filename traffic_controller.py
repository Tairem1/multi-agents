...
class TrafficController:
    ...
    def tick(self):
        ...
        for index, row in self.traffic.iterrows():
            ...
            row['vehicle'].set_control(steering, acceleration)
    ...
    def spawn_car(self, route_index, random_point=False):
        if random_point:
            i = np.random.randint(len(self.world.routes[route_index]))
        else: 
            i = 0
        ...
        if not self.world.collision_exists(car):
            self.world.add(car)
            desired_speed = 5.0 #max(np.random.normal(5.0, 1.0), 1.0)
            controller = CarController(route, desired_speed, car)
